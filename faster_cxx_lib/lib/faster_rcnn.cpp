...
void Detector::Detect(const string& im_name)
{
    ...
    // re-implement img_scale to scale image to 1000x600
    /*
    float img_scale = (float)min_input_side / float(min_side);
    if ((int)(img_scale * max_side) > max_input_side) {
    ...
    */
    ...
    std::cout << "processing image: " << im_name << endl;
    float im_info[3];
    ...
    //cv::resize(cv_new, cv_resized, cv::Size(width, height), cv::INTER_LINEAR);
    ...
    // use memcpy or caffe_gpu_memcpy instead
...
void Detector::vis_detections(cv::Mat image, int* keep, int num_out, float* sorted_pred_cls, float CONF_THRESH)
{
    ...
    while(sorted_pred_cls[keep[i]*5 + 4] > CONF_THRESH && i < num_out)
    {
        if(i >= num_out)
            return;
        cv::rectangle(image,cv::Point(sorted_pred_cls[keep[i]*5 + 0],
                    sorted_pred_cls[keep[i]*5 + 1]),
                    cv::Point(sorted_pred_cls[keep[i]*5 + 2],
                    sorted_pred_cls[keep[i]*5 + 3]),
                    cv::Scalar(255, 0, 0));
        i++;
    }
}
